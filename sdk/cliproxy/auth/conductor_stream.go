	...
	go func() {
		...
		emit := func(chunk cliproxyexecutor.StreamChunk) bool {
			if chunk.Err != nil && !failed {
				...
				m.recordExecutionResult(ctx, Result{AuthID: auth.ID, Provider: provider, Model: resultModel, Success: false, Headers: cloneHTTPHeader(headers), Error: rerr, Options: opts}, auth, ephemeralResult)
			}
			if !forward {
				return false
			}
		...
		if !failed && (ephemeralResult || claudeOAuthRequestCancellation(ctx, auth, nil) == nil) {
			m.recordExecutionResult(ctx, Result{AuthID: auth.ID, Provider: provider, Model: resultModel, Success: true, Headers: cloneHTTPHeader(headers), Options: opts}, auth, ephemeralResult)
		}
	}()
