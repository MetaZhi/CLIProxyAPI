	...
	go func() {
		...
		emit := func(chunk cliproxyexecutor.StreamChunk) bool {
			if chunk.Err != nil && !failed {
				...
				result := Result{AuthID: auth.ID, Provider: provider, Model: resultModel, RouteModel: routeModel, Success: false, Headers: cloneHTTPHeader(headers), Error: rerr, Options: opts}
		...
		if !failed && (ephemeralResult || claudeOAuthRequestCancellation(ctx, auth, nil) == nil) {
			m.recordExecutionResult(ctx, Result{AuthID: auth.ID, Provider: provider, Model: resultModel, RouteModel: routeModel, Success: true, Headers: cloneHTTPHeader(headers), Options: opts}, auth, ephemeralResult)
		}
	}()
...
func (m *Manager) executeStreamWithModelPool(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, routeModel, executionModel string, execModels []string, pooled bool, aliasResult OAuthModelAliasResult, routing *apiKeyModelRoutingSnapshot, allowRetry bool, ephemeralResult bool) (*cliproxyexecutor.StreamResult, error) {
	...
	for idx, execModel := range execModels {
		...
		streamResult, errStream := executeWithSameAuthNetworkRetry(m, ctx, auth, provider, resultModel, "stream-open", func() (*cliproxyexecutor.StreamResult, error) {
		...
		if errStream != nil {
			...
			if errors.As(errStream, &probeErr) {
		...
		if bootstrapErr != nil {
			for sameAttempt := 0; bootstrapErr != nil && sameAttempt < int(m.sameAuthNetworkRetry.Load()); sameAttempt++ {
				retryable, class := sameAuthNetworkRetryableError(bootstrapErr)
				if !retryable {
					break
				}
				if errStorm := m.recordNetworkRetryStormFailure(ctx, auth, provider, resultModel, "stream-bootstrap", class, bootstrapErr); errStorm != nil {
					bootstrapErr = errStorm
					break
				}
				discardStreamChunks(streamResult.Chunks)
				retryStream, retryErr := executeWithSameAuthNetworkRetry(m, ctx, auth, provider, resultModel, "stream-bootstrap", func() (*cliproxyexecutor.StreamResult, error) {
					return executor.ExecuteStream(ctx, auth, execReq, execOpts)
				})
