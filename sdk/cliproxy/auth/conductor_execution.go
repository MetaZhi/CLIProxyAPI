...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int, retryRound int, defaultRequestRetry int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		for _, upstreamModel := range models {
			...
			result := Result{AuthID: auth.ID, Provider: provider, Model: resultModel, RouteModel: routeModel, Success: errExec == nil, Headers: resp.Headers, Options: execOpts}
			if errExec != nil {
				var probeErr *networkRetryProbeError
				if errors.As(errExec, &probeErr) {
					return cliproxyexecutor.Response{}, probeErr.err
				}
				if isNetworkRetrySuppressedError(errExec) {
...
func (m *Manager) executeCountMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int, retryRound int, defaultRequestRetry int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		for _, upstreamModel := range models {
			...
			result := Result{AuthID: auth.ID, Provider: provider, Model: resultModel, RouteModel: routeModel, Success: errExec == nil, Headers: resp.Headers, Options: execOpts, SkipQuotaObservation: true}
			if errExec != nil {
				var probeErr *networkRetryProbeError
				if errors.As(errExec, &probeErr) {
					return cliproxyexecutor.Response{}, probeErr.err
