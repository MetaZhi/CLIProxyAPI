...
func isBuiltInSelector(selector Selector) bool {
	...
	case *RoundRobinSelector, *WeightedRoundRobinSelector, *FillFirstSelector, *QuotaPrioritySelector:
		return true
	default:
		return false
	}
...
func (m *Manager) availableAuthsForSelector(selector Selector, auths []*Auth, provider, routeModel string, now time.Time) (priorityAuths, selectorAuths []*Auth, err error) {
	if _, affinity := selector.(*SessionAffinitySelector); isBuiltInSelector(selector) || affinity {
		quotaWindow, _ := selectorQuotaPriorityExhaustionWindow(selector)
		auths, err = quotaAvailableAuths(auths, provider, routeModel, now, quotaWindow)
		if err != nil {
			return nil, nil, err
		}
	}
...
func schedulerAuthCandidates(auths []*Auth) []pluginapi.SchedulerAuthCandidate {
	...
	for _, auth := range auths {
		...
		if remaining, ok := remainingQuotaPercent(auth); ok {
			candidate.RemainingQuotaPercent = &remaining
		}
