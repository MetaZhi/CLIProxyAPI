...
func normalizedRoutingRuntimeState(cfg *config.Config) routingRuntimeState {
	...
	state.quotaReservePercent = routingQuotaReservePercent(cfg, log.Warnf)
...
func newRoutingSelectorFromRuntimeState(state routingRuntimeState) coreauth.Selector {
	var selector coreauth.Selector
	switch state.strategy {
	case "weighted-round-robin":
		selector = &coreauth.WeightedRoundRobinSelector{}
	case "fill-first":
		selector = &coreauth.FillFirstSelector{QuotaReservePercent: state.quotaReservePercent}
	case "quota-priority":
		...
		if quotaSelector, ok := selector.(*coreauth.QuotaPrioritySelector); ok {
			quotaSelector.QuotaReservePercent = state.quotaReservePercent
		}
	default:
...
func (s *Service) applyManagerConfig(ctx context.Context, commit configCommit) bool {
	...
	if s.appliedRoutingState == nil || *s.appliedRoutingState != routingState {
		s.coreManager.SetSelector(newRoutingSelectorFromRuntimeState(routingState))
